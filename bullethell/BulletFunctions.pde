...
These are the bullet functions, they all determine the patterns that new bullets which spawn from enemies undergo.
 ...
 how many bullets are spawned and if there are to be any changes in their behaviour, such as with the boss. To have easy consistent repetition, the modulus function is used on RealFrame to see when bullettime divides into it
 to make patterns even in time.
 */
