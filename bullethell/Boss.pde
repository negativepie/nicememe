...
class Boss extends Enemy{
 ...
 void act(){
   ...
   y=y+dy;
   if(y>180){
    y=200;                               //gets boss to stop moving down once it reaches y=200
  }
   if(x>700){
     dx=-dx;
           }
 ...
 collisioncheck();                     //makes sure that boss takes bullet damage and kills the player if contact is made
   if(RealFrame%time==0){
   CircleRepeat(x,y,3,3,time,60,0);     //gives the various bullet patterns of the boss, the difference RealFrame ensures that boss cycles through a few different patterns
   }
