...
bullet time which determines the standard time interval for which a new bullet may be spawned and is input into the 'if statements' and functions inside them which govern bullet spawning for a specific enemy.
This, as well as attaching the bullet spawning partially to the Enemy class allows initial bullet positions to be tied to enemy positions.
 */
...
class Enemy extends GameObject {
  ...
  */
  ...
  triggers the collisiondetect function given the input parameters. If such is the case then the enemy loses one hp and the object which collided with the enemy loses its hp (so that the player bullet doesn't continually hit the enemy),
  then particles are spawned where the bullet hit. The necessary parameters for the tempobject are pulled and used.
  */
